...
namespace LdesServer.Administration.Services;
    ...
    IViewRepository viewRepository) : ICollectionService
{
    ...
    public async Task<ValidationResult> CreateCollectionAsync(IGraph definition)
    {
        ...
        var collectionIri =
            definition.WithStandardPrefixes().GetTriplesWithPredicateObject(
                    definition.CreateUriNode(QNames.rdf.type),
                    definition.CreateUriNode(QNames.ldes.EventStream))
                .SingleOrDefault()?.Subject.ToString() ??
