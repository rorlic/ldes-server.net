...
|---------|------|-------------|---------|
| `LdesServer:BaseUri` | string | Base URI for LDES or page URLs in content | `http://localhost:8080/` |
...
**Example:** If your server is accessible at `https://ldes.example.org/` but `BaseUri` is set to `http://localhost:8080/`, clients will try to follow links to localhost and fail.
