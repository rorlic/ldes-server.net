...
When('I request the LDES', () => {
    return cy.request(`${server.baseUrl}/feed/${parkAndRideLdes}`).then(response => ldesResponse = response);
})
...
When('I request the view compressed', () => {
    return cy.request({
        ...
        url: `${server.baseUrl}/feed/${parkAndRideLdes}/${byPage}`,
    }).then(response => compressedViewResponse = response);
})

When('I request the LDES view', () => {
    return cy.request(`${server.baseUrl}/feed/${parkAndRideLdes}/${byPage}`).then(response => viewResponse = response);
})
...
Then('the {string} LDES contains {int} members', (collection: string, count: number) => {
    new Fragment(`${server.baseUrl}/feed/${collection}/${byPage}?pageNumber=1`).visit().then(fragment => waitForFragment(fragment, x => x.memberCount === count, `have member count equal ${count}`));
})
